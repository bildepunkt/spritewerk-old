 ...
 *
 */
function tuneIn(el, type, handler) {
    if (_compliant) {
 ...
 *
 */
function tuneOut(el, type, handler) {
    if (_compliant) {
 ...
 *
 */
function broadcast(el, type, data) {
    var windowType,
        customEvent;
...
var radio = {
    ...
    tuneIn: function(el, type, handler, context) {
        ...
        if (el.hasOwnProperty('length') && el.length > 0) {
            for (var i = 0; i < el.length; i += 1) {
                tuneIn(el[i], type, handlers ? handlers.boundHandler : handler);
            }    
        } else {
            tuneIn(el, type, handlers ? handlers.boundHandler : handler);
        }
    },
    ...
    tuneOut: function(el, type, handler) {
        ...
        if (el.hasOwnProperty('length') && el.length > 0) {
            for (var i = 0; i < el.length; i += 1) {
                tuneOut(el[i], type, handler);
            }    
        } else {
            tuneOut(el, type, handler);
        }
    },
    ...
    broadcast: function(el, type, data) {
        ...
        if (el.hasOwnProperty('length') && el.length > 0) {
            for (var i = 0; i < el.length; i += 1) {
                broadcast(el[i], type, data);
            }    
        } else {
            broadcast(el, type, data);
        }
    },
